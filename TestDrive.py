...
class App(ctk.CTk):
    def __init__(self, *args, **kwargs):
        ...
        PreP_menu.add_command(label="Train Test Split",
                              command=lambda: self.frames[ppf.PrePFrame].show_frame("train_test_split"))
        ...
        PreP_menu.add_command(label='CondensedNearestNeighbour',
                              command=lambda: self.frames[ppf.PrePFrame].show_frame("cnn"))
        PreP_menu.add_command(label='SVMSMOTE')

        # Menu visualization

        vis_menu = menu.menu_bar(text="visualization", tearoff=0)
        vis_menu.add_command(label="visualization with Matplotlib",
                             command=lambda: self.frames[viz.visulateFrame].show_frames("Matplotlib"))
        vis_menu.add_command(label="visualization with Seaborn",
                             command=lambda: self.frames[viz.visulateFrame].show_frames("Seaborn"))
        ...
        for F in (imf.StartFrame, ppf.PrePFrame,viz.visulateFrame):
